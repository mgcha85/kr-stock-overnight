...
Executes automated trading cycle on weekdays (Asia/Seoul time):
  ...
  - 15:30:00 KST: Market Close BUY & Overnight Stock Selection Telegram Alert
  - 15:35:00 KST: Post-Market Parity Verification Telegram Alert
"""
...
def run_scheduler():
    logger.info("=========================================================================")
    logger.info("   KRX PAPER TRADING DAEMON SCHEDULER STARTED")
    logger.info("   Timezone: Asia/Seoul (KST)")
    logger.info("   Schedule: Weekdays Mon-Fri [09:00 SELL | 15:30 BUY | 15:35 PARITY]")
    logger.info("=========================================================================")
    ...
    while running:
        ...
        if weekday < 5:
            ...
            # 2. Afternoon 15:30:00 ~ 15:34:00 Market Close BUY
            if "15:30:00" <= time_str <= "15:34:00" and bought_date != today_str:
                logger.info(f"[{today_str} {time_str}] Executing 15:30 Market Close BUY...")
                try:
                    engine.execute_market_close_buy(today_str)
                    bought_date = today_str
                    logger.info(f"[{today_str}] 15:30 Market Close BUY Completed Successfully.")
                except Exception as e:
                    logger.error(f"[{today_str}] Error during 15:30 Market Close BUY: {e}", exc_info=True)

            # 3. Post-Market 15:35:00 ~ 15:40:00 Parity Verification
            if "15:35:00" <= time_str <= "15:40:00" and parity_date != today_str:
                logger.info(f"[{today_str} {time_str}] Executing 15:35 Post-Market Parity Verification...")
                try:
                    engine.run_post_market_parity_check(today_str)
                    parity_date = today_str
                    logger.info(f"[{today_str}] 15:35 Parity Verification Completed Successfully.")
                except Exception as e:
                    logger.error(f"[{today_str}] Error during 15:35 Parity Verification: {e}", exc_info=True)
